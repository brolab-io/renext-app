...
const UtilPage = () => {
  ...
  const createTokenMint = async () => {
    ...
    const quantity = new BN(amount.toString()).mul(new BN(10).pow(new BN(decimals.toString())));

    if (quantity.toString().length > 20) return toast.error("Quantity too large");
    ...
    toast.promise(
      new Promise(async (resolve, reject) => {
        ...
        const transferInstruction = Token.createTransferInstruction(
          ...
          publicKey,
          [],
          Number(new BN(amount.toString()).mul(new BN(10).pow(new BN(decimals.toString()))))
        );
