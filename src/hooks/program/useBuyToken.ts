...
import { CAMPAIGN_TYPE_POOL, CURRENCY_POOL, buyWithReUSD, buyWithReUSDAnWhitelist, buyWithRenec, buyWithRenecAndWhitelist } from "@/utils/program.util";
...
const useBuyToken = (launch_pool_pda: string) => {
    ...
    return useMutation(
        ...
        async (amount: string) => {
            ...
            const minCanBuy = poolData.minimumTokenAmount.div(new BN(10).pow(new BN(poolData.tokenMintDecimals)));
            const maxCanBuy = poolData.maximumTokenAmount.div(new BN(10).pow(new BN(poolData.tokenMintDecimals)));

            if (amount < minCanBuy || amount > maxCanBuy) {
                return Promise.reject(new Error(`Amount should be between ${minCanBuy} and ${maxCanBuy}`));
            }



            if (CURRENCY_POOL.RENEC in poolData.currency && CAMPAIGN_TYPE_POOL.FairLaunch in poolData.poolType) {
            ...
            } else if (CURRENCY_POOL.REUSD in poolData.currency && CAMPAIGN_TYPE_POOL.FairLaunch in poolData.poolType) {
            ...
            } else if (CURRENCY_POOL.RENEC in poolData.currency && CAMPAIGN_TYPE_POOL.Whitelist in poolData.poolType) {
            ...
            } else if (CURRENCY_POOL.REUSD in poolData.currency && CAMPAIGN_TYPE_POOL.Whitelist in poolData.poolType) {
                console.log('usd in whitelist')
                const tx = await buyWithReUSDAnWhitelist(program,
                    new PublicKey(launch_pool_pda),
                    wallet.publicKey,
                    amount
                )
