...
import { withdrawNativePool, withdrawTokenPool } from "@/utils/program.util";
    ...
    return useMutation(
        ...
        async () => {
            ...
            if (currency.toUpperCase() === "REUSD") {
                const { tx } = await withdrawTokenPool(program, new PublicKey(launch_pool_pda), wallet.publicKey, wallet.publicKey);
                return {
                    tx,
                };
            }
            if (currency.toUpperCase() === "RENEC") {
                const { tx } = await withdrawNativePool(program, new PublicKey(launch_pool_pda), wallet.publicKey, wallet.publicKey);
                return {
                    tx,
                };
            }
