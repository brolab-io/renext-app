...
import { findLaunchPoolAccount, findMintTokenAccount, findTreasurerAccount, findUserPoolAccount, findVaultAccount, findWhitelistAccount } from "./account.util";
...
export async function createNativePool(program: Program<RenextProgram>, creator: PublicKey, payload: any) {
    ...
    console.log(`launch_pool: ${launch_pool.toBase58()} creator: ${creator.toBase58()} with mint: ${mint.toBase58()} creating ....`)
    console.log('--------------------------------------')
    ...
    return { tx, result: { id: tx } };
}

export async function createTokenPool(program: Program<RenextProgram>, creator: PublicKey, payload: any) {
    const mint = new PublicKey(payload.token_address);
...
export async function startLaunchPool(program: Program<RenextProgram>, creator: PublicKey, pool: PublicKey) {
    ...
    console.log(`launch_pool: ${launch_pool.toBase58()} creator: ${creator.toBase58()} with mint: ${mint.toBase58()} starting ....`)
    console.log('--------------------------------------')
...
export async function startLaunchPoolWithWhitelist(program: Program<RenextProgram>, creator: PublicKey, pool: PublicKey, wallets: PublicKey[], max_size = 10) {
    const launch_pool = new PublicKey(pool);
    const poolData = await program.account.launchPool.fetch(launch_pool);
    const mint = new PublicKey(poolData.tokenMint);
...
export async function buyWithRenec(program: Program<RenextProgram>, pool: PublicKey, buyer: PublicKey, amount: number | string,) {
    const launch_pool = new PublicKey(pool);
    const poolData = await program.account.launchPool.fetch(launch_pool);
    const mint = new PublicKey(poolData.tokenMint);
    ...
    console.log(`buyer ${buyer.toBase58()} want buy ${amount} token with renec at launch pool ${launch_pool.toBase58()}`);
    console.log('--------------------------------------')
    const tx = await program.methods
        .buyTokenWithNative(
...
export async function buyWithReUSD(program: Program<RenextProgram>, pool: PublicKey, buyer: PublicKey, amount: number | string,) {
    const launch_pool = new PublicKey(pool);
    const poolData = await program.account.launchPool.fetch(launch_pool);
    const reusd_mint = new PublicKey(process.env.NEXT_PUBLIC_REUSD_MINT!);
    ...
    const accountInfo = await program.provider.connection.getAccountInfo(new PublicKey(userTokenAccount));
    if (!accountInfo) {
    ...
    console.log(`buyer ${buyer.toBase58()} want buy ${amount} token with ReUSD ${reusd_mint} at launch pool ${launch_pool.toBase58()}`);
    console.log('--------------------------------------')
...
export async function buyWithRenecAndWhitelist(program: Program<RenextProgram>, pool: PublicKey, buyer: PublicKey, amount: number | string) {
    const launch_pool = new PublicKey(pool);
    const poolData = await program.account.launchPool.fetch(launch_pool);
...
export async function buyWithReUSDAnWhitelist(program: Program<RenextProgram>, pool: PublicKey, buyer: PublicKey, amount: number | string) {
    const launch_pool = new PublicKey(pool);
    const poolData = await program.account.launchPool.fetch(launch_pool);
    const reusd_mint = new PublicKey(process.env.NEXT_PUBLIC_REUSD_MINT!);
    const [user_pool] = findUserPoolAccount(
...
export async function completeLaunchPool(
    ...
    console.log(`launch pool ${launch_pool.toBase58()} run to completed by ${creator.toBase58()} with mint ${poolData.tokenMint.toBase58()}`)
    console.log('--------------------------------------')
...
export async function claimToken(
    ...
    console.log(`buyer ${buyer.toBase58()} want claim ${data.amount.toNumber()} token ${poolData.tokenMint.toBase58()} at launch pool ${launch_pool.toBase58()}`);
    console.log('--------------------------------------')
...
export async function withdrawNativePool(program: Program<RenextProgram>, launch_pool: PublicKey, creator: PublicKey, beneficiary: PublicKey) {
    const poolData = await program.account.launchPool.fetch(launch_pool);
    ...
    console.log(`User ${program.provider?.publicKey?.toBase58()} want withdraw ${accountInfo && accountInfo.lamports / LAMPORTS_PER_SOL} RENEC of launch pool ${launch_pool.toBase58()} with mint ${poolData.tokenMint.toBase58()} from vault ${vault.toBase58()} to beneficiary ${beneficiary.toBase58()}`)
    console.log('--------------------------------------')
...
export async function withdrawTokenPool(program: Program<RenextProgram>, launch_pool: PublicKey, creator: PublicKey, beneficiary: PublicKey) {
    const poolData = await program.account.launchPool.fetch(launch_pool);
    const REUSD_MINT = new PublicKey(process.env.NEXT_PUBLIC_REUSD_MINT!);
    const launchPoolTokenAccount = await findMintTokenAccount(
        launch_pool,
    ...
    const { value } = await await program.provider.connection.getTokenAccountBalance(launchPoolTokenAccount);
    ...
    console.log(`User ${program.provider?.publicKey?.toBase58()} want withdraw ${value.uiAmountString} ReUSD of launch pool ${launch_pool.toBase58()} with mint ${poolData.tokenMint.toBase58()} from vault ${launchPoolTokenAccount.toBase58()} to beneficiary ${beneficiary.toBase58()}`)
    console.log('--------------------------------------')
