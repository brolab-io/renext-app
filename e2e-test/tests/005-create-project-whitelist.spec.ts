...
test('Create Project Whitelist', async ({ page }) => {
 ...
 ;
  ...
  // Upload whitelist
  const wallet_buy = process.env.WALLET_BUY;
  await page.getByRole('button', { name: 'Start Pool with Whitelist' }).click();
  await page.locator('#file-upload').first().setInputFiles({
    name: 'file.csv',
    mimeType: 'text/plain',
    buffer: Buffer.from(`Address\n${wallet_buy}\nGDQpy87L2Sx4C62nh6J7uALsrNQj8pLZoei1oQw7tbtR\n`)
  });
  ...
  await page.waitForTimeout(5000);
  await page.reload();
  const white_list_wallet = await page.getByRole('link', { name: wallet_buy }).innerText();
  expect(white_list_wallet).toEqual(wallet_buy);
