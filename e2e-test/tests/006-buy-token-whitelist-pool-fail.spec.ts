...
const fair_launch_pool = process.env.FAIRLAUNCH_POOL;

test('Buy Token Whitelist Fail', async ({ page }) => {
  ...
  await page.getByPlaceholder('Name').fill('acc 3');
  await page.getByPlaceholder('Private key').click();
  await page.getByPlaceholder('Private key').fill(process.env.WALLET_BUY_PRIVATE_2);
